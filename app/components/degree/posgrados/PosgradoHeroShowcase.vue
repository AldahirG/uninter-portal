...
const getDegreePrefix = (name?: string) => {
  if (!name) return "POSGRADO";
  const lower = name.toLowerCase();
  if (lower.includes("especialidad")) return "ESPECIALIDAD EN";
  if (lower.includes("maestría") || lower.includes("maestria")) return "MAESTRÍA EN";
  if (lower.includes("doctorado")) return "DOCTORADO EN";
  return "POSGRADO EN";
};

const processDegreeName = (name?: string) => {
  if (!name) return "";
      ...
      <div class="degree-hero__bg">
        <picture v-if="data?.imageHorizontal && data?.imageVertical" class="degree-hero__picture">
          <source
            ...
            :srcset="encodeURI(`/images/posgrados/Web Vertical/${data.imageVertical}`)"
          />
          <img
            :src="encodeURI(`/images/posgrados/Web Horizontal/${data.imageHorizontal}`)"
            :alt="data.name"
      ...
      <div class="degree-container degree-hero__content">
        <div class="degree-hero__inner">
          ...
          <p class="degree-hero__eyebrow">
            {{ getDegreePrefix(data?.name) }}
          </p>

          <!-- Título principal -->
          <h1 class="degree-hero__title" :class="{'long-title': processDegreeName(data?.name).length > 20}">
            {{ processDegreeName(data?.name) }}
          </h1>
    ...
    <div class="degree-hero__stats-bar">
      <div class="degree-container degree-hero__stats-inner">
        ...
        <div class="degree-stat">
          ...
          <div class="degree-stat__text">
            <span class="degree-stat__value">{{ (data as any)?.formato || 'Híbrido' }}</span>
...
.degree-btn-card:hover {
  background: rgba(123, 158, 53, 0.2);
  border-color: #8cb33e;
  color: #8cb33e;
  transform: translateY(-2px);
  box-shadow: 0 8px 24px rgba(123, 158, 53, 0.25);
}
...
/* ═══ STATS BAR (LIQUID GLASS VERDE OLIVA) ═══ */
.degree-hero__stats-bar {
  ...
  background-image: radial-gradient(
    circle at 50% 50%,
    rgba(123, 158, 53, 0.15) 0,
    transparent 60%
  );
  background-attachment: fixed;
  border-top: 1px solid rgba(139, 179, 62, 0.3);
...
.degree-stat {
  ...
  gap: 0.85rem;
  padding: 0 1rem;
  border-right: 1px solid rgba(139, 179, 62, 0.3);
}
